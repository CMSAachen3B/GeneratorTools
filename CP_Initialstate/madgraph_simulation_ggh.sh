...
chmod u+x $MADGRAPH_DIR/JJ_X0/SubProcesses/makefile

sed -i -e "s@\(F2PY.*\)\$@\1 --fcompiler=gnu95@g" $MADGRAPH_DIR/JJ_X0/SubProcesses/makefile
...
sed -i -e "s@\(F2PY.*\)\$@\1 --fcompiler=gnu95@g" $MADGRAPH_DIR/JJ_X0JJJ/SubProcesses/makefile

for makefile in $MADGRAPH_DIR/JJ_X0JJJ/SubProcesses/P*/makefile;
do
	echo `dirname $makefile`
	echo "before"
	cd `dirname $makefile` 
