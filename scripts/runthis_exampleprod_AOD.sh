...
cp $STARTDIR/kappaWorkflow_privateMiniAOD_AOD.sh $WORKDIR/
...
if [ $USECRAB = "True" ]; then
	...
	sed -e "s/#NUMBEREVENTS#/${NUMBEREVENTS}/g" $STARTDIR/crabConfig_tutorial_MC_generation3_wscript_AOD.py > ./crabconfig_eventsInserted.py
