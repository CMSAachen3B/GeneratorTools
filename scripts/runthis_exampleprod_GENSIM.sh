...
echo $NUMBEREVENTS
sed -e "s/#NUMBEREVENTS#/${NUMBEREVENTS}/g" $STARTDIR/GENSIM.py > ./pythonGENSIM_cfg_eventsInserted.py
...
if [ $USECRAB = "True" ]; then
	...
	sed -e "s~#LHELOCATION#~../LHE.root~g" ./pythonGENSIM_cfg_eventsInserted.py > ./pythonGENSIM_cfg.py
	...
	sed -e "s/#NUMBEREVENTS#/${NUMBEREVENTS}/g" $STARTDIR/crabConfig_tutorial_MC_generation3_wscript_GENSIM.py > ./crabconfig_eventsInserted.py
